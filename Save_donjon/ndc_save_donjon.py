...
class Joueur:
    ...
    def draw(self):
        ...
        if self.acceuil == False:
            ...
            # pyxel.text(2, 10, str(self.x), 7)
            # pyxel.text(2, 20, str(self.y), 7)
            # pyxel.text(2, 30, str(pyxel.mouse_x), 7)
            # pyxel.text(2, 40, str(pyxel.mouse_y), 7)
            if pyxel.btn(pyxel.KEY_A):
                pyxel.line(self.x+6, self.y+5, pyxel.mouse_x, pyxel.mouse_y, 7)
            # tir
...
# depalcement
    ...
    def on_mouse_down(x, y, button, modifiers):
        print("Clic à la position (x={}, y={})".format(x, y))
